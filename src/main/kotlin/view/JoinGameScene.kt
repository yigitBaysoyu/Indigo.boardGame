 ...
 */
class JoinGameScene(private val rootService: RootService) : MenuScene(Constants.SCENE_WIDTH, Constants.SCENE_HEIGHT), Refreshable {
    ...
    private val playerNameLabelList = mutableListOf<Label>().apply {
        for(i in 0 until 4) {
    ...
    val leaveButton = Button( // TODO add logic in IndigoApplication
        width = 350, height = 75,
    ...
    init {
        ...
        addComponents(
            cornersBackground,
            headerLabel,
            playerNameLabelList[0],
            playerNameLabelList[1],
            playerNameLabelList[2],
            playerNameLabelList[3],
    ...
    private fun checkForSamePlayerColors(): Boolean {
        ...
        for(i in 0 until 4) {
            if(!playerNameLabelList[i].isVisible) break
    ...
    private fun isPlayerNameLengthValid(index: Int): Boolean {
        if(!playerNameLabelList[index].isVisible) return true
        val name = playerNameLabelList[index].text
    ...
    private fun resetAllComponents() {
        ...
        for(i in 0 until 4) {
            ...
            playerNameLabelList[i].text = "Player ${i + 1}"
        ...
        for(i in 2 until 4) {
            playerNameLabelList[i].isVisible = false
